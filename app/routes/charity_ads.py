...
async def list_charity_ads(
    ...
    try:
        ...
        for a in result.data or []:
            ...
            if ad_type in ["tourism", "pet", "job", "job_ad", "employment", "moving_sale", "moving", "regulation", "question", "tip"]:
                continue
            title = str(a.get("title") or "")
            desc = str(a.get("description") or "")
            # Se tiver prefixo de colchetes ou for remanescente de teste de emprego ou mudança ou regularização
            if (title.startswith("[") and "]" in title) or "[meta_job]" in desc.lower() or "[emprego]" in title.lower() or "moving_meta:" in desc.lower() or "[mudança]" in title.lower() or "[mudanca]" in title.lower() or "regulation_meta:" in desc.lower() or "[question]" in title.lower() or "[tip]" in title.lower() or "[duvida]" in title.lower() or "[dica]" in title.lower():
                continue
        ...
        try:
            ...
            for a in res_all.data or []:
                ...
                desc = str(a.get("description") or "")
                if (t.startswith("[") and "]" in t) or "[meta_job]" in desc.lower() or "[emprego]" in t.lower() or "moving_meta:" in desc.lower() or "[mudança]" in t.lower() or "[mudanca]" in t.lower() or "regulation_meta:" in desc.lower() or "[question]" in t.lower() or "[tip]" in t.lower() or "[duvida]" in t.lower() or "[dica]" in t.lower():
                    continue
